...
public:
	...
	/// DestroyParticleInShape(b2ShapeId, const b2Transform&,bool).
	/// @param Shape which encloses particles that should be destroyed.
	/// @param Transform applied to the shape.
	/// @warning This function is locked during callbacks.
	/// @return Number of particles destroyed.
	int32 DestroyParticlesInShape(const b2LiquidShape& shape, const b2Transform& xf)
	{
	...
	int32 GetInternalAllocatedCapacity() const;
	...
	void QueryAABB(b2LiquidQueryCallback* callback, const b2AABB& aabb) const;
	...
	void QueryShapeAABB(b2LiquidQueryCallback* callback, b2ShapeId shape,
...
private:
	...
	b2ParticleSystem(const b2ParticleSystemDef* def, b2LiquidWorld* world);
	~b2ParticleSystem();
...
private:
	...
	///CORONASDK/CORONALABS NOTE: WE CHANGED THIS FROM "private" TO "public".
public:
	...
	const b2ParticleContact& rhs) const
{
	...
	return indexA == rhs.indexA
		&& indexB == rhs.indexB
		&& flags == rhs.flags
		&& b2AbsFloat(weight - rhs.weight) < MAX_WEIGHT_DIFF
...
inline int32 b2ParticleSystem::GetInternalAllocatedCapacity() const
{
	return m_internalAllocatedCapacity;
}
