...
namespace // anonymous namespace.
{
	...
	bool liquid_query_callback(b2ShapeId shapeId, void* context)
	{
		...
		if ( ! b2Body_IsValid(bodyId) || b2Shape_IsSensor(shapeId) ) {
			return true;
		}

		b2AABB aabb = b2Shape_GetAABB(shapeId);
		b2ParticleSystem* system = queryContext->system;
		b2ParticleSystem::InsideBoundsEnumerator enumerator = system->GetInsideBoundsEnumerator(aabb);
		int32 index = -7;
	...
	bool liquid_query_update_body_contacts_callback(b2ShapeId shapeId, void* context)
	{
		b2ParticleSystem* system = static_cast<b2ParticleSystem*>(context);
		...
		b2ParticleSystem::InsideBoundsEnumerator enumerator = system->GetInsideBoundsEnumerator(aabb);
		int32 a = -9;
		while ((a = enumerator.GetNext()) >= 0)
		{
			b2Vec2 ap = system->m_positionBuffer.data[a];
			float32 d;
			b2Vec2 n;
			...
			if (d < system->m_particleDiameter && liquid_should_collide(system, shapeId, a))
			{
				...
				float32 invAm =
					...
					b2_wallParticle ? 0 : system->GetParticleInvMass();
				b2Vec2 rp = ap - bp;
				float32 rpn = b2Cross(rp, n);
				float32 invM = invAm + invBm + invBI * rpn * rpn;

				b2ParticleBodyContact& contact =
					system->m_bodyContactBuffer.Append();
				contact.index = a;
				contact.bodyId = bodyId;
				contact.fixture = shapeId;
				contact.weight = 1 - d * system->m_inverseDiameter;
				contact.normal = -n;
				contact.mass = invM > 0 ? 1 / invM : 0;
				system->DetectStuckParticle(a);
			}
		}
		// Rtt_Log("particle_system.cpp -> liquid_query_update_body_contacts_callback, a = %d", a);
...
public:
	...
	bool operator()(const b2ParticleBodyContact& contact)
	{
		...
		if (!b2Shape_TestPoint(contact.fixture, pos))
		{
			// b2Body_GetShapeCount(b2BodyId bodyId)
...
public:
	...
	FixedSetAllocator(b2LiquidStackAllocator* allocator);
...
private:
	...
	b2LiquidStackAllocator* m_allocator;
};
...
public:
	...
	TypedFixedSetAllocator(b2LiquidStackAllocator* allocator) :
...
typedef LightweightPair<b2ShapeId,int32> FixtureParticle;
...
public:
	// Initialize members of this class.
	FixtureParticleSet(b2LiquidStackAllocator* allocator) :
...
public:
	// Initialize members of this class.
	b2ParticlePairSet(b2LiquidStackAllocator* allocator) :
...
struct QueryParticleContext
{
	int fResultCount;
};

int32 b2ParticleSystem::DestroyParticlesInShape(
	const b2LiquidShape& shape, const b2Transform& xf,
	bool callDestructionListener)
{
	...
	class DestroyParticlesInShapeCallback : public b2LiquidQueryCallback
	{
	public:
		DestroyParticlesInShapeCallback(
			b2ParticleSystem* system, const b2LiquidShape& shape,
	...
	private:
		...
		const b2LiquidShape* m_shape;
		b2Transform m_xf;
		bool m_callDestructionListener;
		int32 m_destroyed;
	} callback(this, shape, xf, callDestructionListener);
	b2AABB aabb = shape.ComputeAABB(xf);
	// b2Body_ComputeAABB( b2Shape_GetBody(shape))
	...
	const b2ParticleGroupDef& groupDef, const b2Transform& xf)
{
		...
		y < aabb.upperBound.y; y += stride)
	{
			...
			x < aabb.upperBound.x; x += stride)
		{
			...
			{
				CreateParticleForGroup(groupDef, b2Transform_identity, p);
			}
		}
	}
}
...
FixedSetAllocator::FixedSetAllocator(
		b2LiquidStackAllocator* allocator) :
...
private:
	...
	bool ReportFixture(b2Fixture* fixture)
	{
		...
		b2ShapeId shape = fixture->GetShape();
		int32 childCount = shape->GetChildCount();
		for (int32 childIndex = 0; childIndex < childCount; childIndex++)
		{
...
void b2ParticleSystem::SolveCollision(const b2TimeStep& step)
{
	...
	// m_world->QueryAABB(&callback, aabb);
	// Rtt_Log("SolveCollision: aabb center(%f, %f)", b2AABB_Center(aabb).x, b2AABB_Center(aabb).y);
	LiquidQueryContext context = { this, step };
	// b2World_OverlapAABB(m_world->GetWorldId(), aabb, b2DefaultQueryFilter(), liquid_query_callback, &context);
	m_world->QueryAABB(aabb, b2DefaultQueryFilter(), liquid_query_callback, &context);
}
...
{
	b2Vec2 gravity = step.dt * m_def.gravityScale * b2World_GetGravity(m_world->GetWorldId());
	for (int32 i = 0; i < m_count; i++)
	{
...
void b2ParticleSystem::SolvePressure(const b2TimeStep& step)
{
	...
	for (int32 k = 0; k < m_bodyContactBuffer.GetCount(); k++)
	{
		...
		b2Body_ApplyLinearImpulse(contact.bodyId, f, p, true);
	}
	for (int32 k = 0; k < m_contactBuffer.GetCount(); k++)
	{
		const b2ParticleContact& contact = m_contactBuffer[k];
...
void b2ParticleSystem::SolveDamping(const b2TimeStep& step)
{
	...
	for (int32 k = 0; k < m_bodyContactBuffer.GetCount(); k++)
	{
		...
		if (vn < 0)
		{
			...
			m_velocityBuffer.data[a] += GetParticleInvMass() * f;
			// b->ApplyLinearImpulse(-f, p, true);
			b2Body_ApplyLinearImpulse(b, -f, p, true);
		}
	}
	for (int32 k = 0; k < m_contactBuffer.GetCount(); k++)
	{
	...
	for (int32 k = 0; k < m_bodyContactBuffer.GetCount(); k++)
	{
		...
		if (IsRigidGroup(aGroup))
		{
			...
			// the particle into the body.
			{
				...
				b2Body_ApplyLinearImpulse(b, -f * n, p, true);
			}
		}
	}
	for (int32 k = 0; k < m_contactBuffer.GetCount(); k++)
...
void b2ParticleSystem::SolveRigid(const b2TimeStep& step)
{
	for (b2ParticleGroup* group = m_groupList; group; group = group->GetNext())
	{
		if (group->m_groupFlags & b2_rigidParticleGroup)
		{
			...
			for (int32 i = group->m_firstIndex; i < group->m_lastIndex; i++)
			{
				m_velocityBuffer.data[i] = b2TransformPoint(velocityTransform,
								  ...
								  const b2Vec2& force)
{
	...
	const b2Vec2 distributedForce = force * ( 1.0f / (float32)(lastIndex - firstIndex) );
										  ...
										  const b2Vec2& impulse)
{
	...
	const b2Vec2 velocityDelta = impulse * (1.0f / totalMass);
	for (int32 i = firstIndex; i < lastIndex; i++)
	{
...
void b2ParticleSystem::QueryAABB(b2LiquidQueryCallback* callback,
...
void b2ParticleSystem::QueryShapeAABB(b2LiquidQueryCallback* callback,
									  b2ShapeId shape,
